...
def parse_date_text(date_raw: str) -> str:
    ...
    if "Published Date:" in date_raw:
        date_part = date_raw.split(":", 1)[1].strip()
    else:
        date_part = date_raw
...
async def fetch_detail_content(page, link):
    for attempt in range(RETRY_TIMES):
        try:
            ...
            content_div = soup.select_one("div._mas-typeset") or soup.select_one("div.mas-rte-content")
            if content_div:
                paragraphs = []
                for elem in content_div.find_all(["p", "h1", "h2", "h3", "h4", "li", "td", "th"]):
                    t = elem.get_text(separator=" ", strip=True)
                    if t and not re.match(r"^\s*(\*{3}|End\s+of\s+Document)\s*$", t, re.I):
                        paragraphs.append(t)
                content_text = "\n\n".join(paragraphs)
            else:
                content_text = ""
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        while count < MAX_ARTICLES and not stop_early:
            ...
            posts = soup.select("article.mas-search-card")
            if not posts:
                break
            ...
            for post in posts:
                ...
                link_tag = post.select_one("a.mas-link--no-underline")
                if not link_tag:
                    continue
                ...
                date_div = post.select_one("div.ts\\:xs")
                ...
                title_span = link_tag.select_one("span.mas-link__text")
                title = title_span.get_text(strip=True) if title_span else ""
                abs_p = post.select_one(".mas-search-card__body p")
            ...
            # Pagination
            try:
                current_page_tag = await page.query_selector('a[role="button"][aria-current="page"]')
                if current_page_tag:
                    current_text = await current_page_tag.text_content()
                    current_num = int(current_text.strip())
                    next_num = current_num + 1
                else:
                    next_num = 2
                
                next_selector = f'a[role="button"][aria-label*="Page {next_num}"]'
